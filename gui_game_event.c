...
static void      gui_game_event_new_node  (eventq_t *eventq);
static eventq_t* gui_game_event_new_eventq();
...
static void gui_game_event_new_node(eventq_t *eventq)
{
  ...
  if( !(node=malloc(sizeof(eventq_node_t))) ) {
    Error("Failed to allocate space (%u) for new eventq node.\n",sizeof(eventq_node_t));
  }

  // Setup the new node for insertion
  memset(node,0,sizeof(eventq_node_t));
  node->next = eventq;
...
static eventq_t* gui_game_event_new_eventq()
{
  ...
  // Allocate eventq
  if( !(eventq=malloc(sizeof(eventq_t))) ) {
    Error("Failed to allocate space (%u) for new eventq.\n",sizeof(eventq_t));
  }
  memset(eventq,0,sizeof(eventq_t));
  eventq->next = eventq;
  eventq->last = eventq;
...
eventq_t* gui_game_event_get(eventq_t *node)
{
  gui_game_event_init();
